...
class AnalyticsService:
    ...
    async def track_user_event(self, event_data: Dict[str, Any]) -> None:
        ...
        try:
            table_id = f"{self.project_id}.leafloaf_analytics.user_events"
            ...
            # Add properties as individual fields for important events
            properties = event_data.get("properties", {})
            if event_data["event_type"] == "intent_analysis":
                row.update({
            ...
            elif event_data["event_type"] == "search_executed":
                row.update({
                    "query": properties.get("query", ""),
                    "results_count": properties.get("results_count", 0),
                    "search_latency_ms": int(properties.get("search_latency_ms", 0))
                })
    ...
    async def track_search_event(self, search_data: Dict[str, Any]) -> None:
        ...
        try:
            table_id = f"{self.project_id}.leafloaf_analytics.search_events"
    ...
    async def track_cart_event(self, cart_data: Dict[str, Any]) -> None:
        ...
        try:
            table_id = f"{self.project_id}.leafloaf_analytics.cart_events"
    ...
    async def track_order_event(self, order_data: Dict[str, Any]) -> None:
        ...
        try:
            table_id = f"{self.project_id}.leafloaf_analytics.order_events"
    ...
    async def _track_promotion_usage(self, order_id: str, user_id: str, 
        ...
        """Track promotion usage"""
        try:
            table_id = f"{self.project_id}.leafloaf_analytics.promotion_usage"
            rows = []
